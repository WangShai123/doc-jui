import {
  ...
  menu,
  lazyRender,
} from '../dist/jui.module.js'
import menuItems from './menus.js'
import locales from './locales.js'
const { show, info, success, warning, error, lite } = toast
...
const __menu = () => {
  return service.get('menu', () => {
    const menuContainer = newEl('section', {
      className: 'flex flex-col items-center justify-center pt-5 menu-container',
      style: {
        'padding-bottom': '3rem',
      },
    ...
    menuContainer.after(
      newEl('div', {
        style: { 'text-align': 'center', padding: '1rem 0', color: 'var(--gray-9)' },
        html: '@JEALER',
      })
    )
  })
...
const svg = name => {
  ...
  switch (name) {
    case 'menu':
      path = '<path d="M3 4H21V6H3V4ZM3 11H21V13H3V11ZM3 18H21V20H3V18Z"></path>'
      break
    case 'palette':
      path =
    ...
    case 'warning':
      ...
      break
    case 'loader':
      path = '<path d="M12 3C16.9706 3 21 7.02944 21 12H19C19 8.13401 15.866 5 12 5V3Z"></path>'
      break
    default:
      path = ''
      break
...
const __modal = () => {
  ...
  const custom = q('.example-modal-custom')
  const custom2 = q('.example-modal-custom-2')
  ...
  if (basic) {
    const basicModal = service.get('basicModal', () => {
      return new modal({
        ...
        onShown: function () {
          success(t('modalShown'))
        },
        onHide: function () {
          info(t('closeModal'))
        },
        onHidden: function () {
          warning(t('modalHidden'))
        },
        onConfirm: function () {
          success(t('modalConfirmed'))
        },
      })
    })
    on(basic, 'click', () => {
  ...
  if (custom) {
    const customModal = service.get('customModal', () => {
      return new modal({
        header: false,
        footer: false,
        content: `<div class="flex flex-col align-center gap-4">
          <div style="width:24px">${svg('warning')}</div><div>${t('Confirm Logout?')}</div>
          <div class="flex gap-4"><button class="j-button is-ghost is-sm" data-action="close">${t('Cancel')}</button><button class="j-button is-primary is-sm" data-action="confirm">${t('Confirm')}</button></div>
          </div>`,
        bgClose: true,
        onConfirm: () => {
  ...
  if (custom2) {
    const customModal = service.get('customModal-2', () => {
      return new modal({
        header: false,
        footer: false,
        content: `<div class="flex flex-col align-center gap-4" style="width:276px">
          <div class="is-active pt-2" style="width:32px"><div class="animate-spin">${svg('loader')}</div></div>
          <div class="font-semiBold">${t('Processing your request')}</div>
          <div class="text-center">${t('Please wait while we process your request. Do not refresh the page.')}</div>
          <div class="flex gap-4 w-1/2 my-2"><button class="j-button is-contrast is-sm w-full" data-action="cancel">${t('Cancel')}</button></div>
          </div>`,
        onHide: () => {
          info(`${t('Cancel Success')}`)
        },
      })
    })
    on(custom2, 'click', () => {
...
const __mobileMenu = () => {
  const _menu = new menu('#mobile-menu', {
    backText: t('Back'),
  })
  _menu.build()
}
