...
Content is the rendering container for page text.

`j-content` is a class for handling WYSIWYG-generated content that contains only `HTML` tags. This class can be used in any scenario where text needs to be written.
...
Visit [Component Preview](/dashboard.html) to see the four size formats of content.
