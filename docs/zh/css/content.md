...
内容域（Content）是页面文本的渲染容器。

`j-content` 是一个用于处理所见即所得（WYSIWYG）生成内容的类，其中仅包含 `HTML` 标签。在任何需要编写文本的场景下，都可以使用这个类。
...
访问 [组件预览](/dashboard.html)，查看内容域的四种尺寸格式。

## 基础示例

<!-- <div class="j-radio is-group my-4" id="radio">
    <label class="radio-label">
        <input type="radio" class="j-input" name="content" value="sm">
        <span class="radio-text">is-sm</span>
    </label>
    <label class="radio-label">
        <input type="radio" class="j-input" name="content" value="md" checked>
        <span class="radio-text">is-md</span>
    </label>
    <label class="radio-label">
        <input type="radio" class="j-input" name="content" value="lg">
        <span class="radio-text">is-lg</span>
    </label>
    <label class="radio-label">
        <input type="radio" class="j-input" name="content" value="xl">
        <span class="radio-text">is-xl</span>
    </label>
</div> -->

<div id="demo" class="j-content is-md">
  <h1>Hello World</h1>
  <p>
